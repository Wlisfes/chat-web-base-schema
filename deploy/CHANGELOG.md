...
- Change: `NacosModule.forRoot()` now requires the complete flat bootstrap, subscription and registration contract. The shared runtime no longer reads `NACOS_*` or `server.port` from `ConfigService`.
