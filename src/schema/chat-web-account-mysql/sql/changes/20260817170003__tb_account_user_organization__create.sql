CREATE TABLE IF NOT EXISTS `tb_account_user_organization` (
    ...
    `organization_uid` varchar(19) NOT NULL COMMENT '组织UID',
    ...
    PRIMARY KEY (`key_id`),
    UNIQUE KEY `uk_tb_account_user_organization_member` (`user_uid`, `organization_uid`),
    KEY `idx_tb_account_user_organization_org_status` (`organization_uid`, `status`)
) ENGINE = InnoDB
