...
const requestStorage = new AsyncLocalStorage<RequestContextStore>()
...
export const requestContextMiddleware: RequestHandler = (request, response, next) => {
    ...
    requestStorage.run({ requestId, request: request as RequestContextCarrier }, next)
}
...
export function getActiveExecutionMethod(): string | undefined {
    const store = requestStorage.getStore()
    return store?.executionMethod ?? store?.request?.executionMethod
}

export function setActiveExecutionMethod(executionMethod: string): void {
    const store = requestStorage.getStore()
    if (!store) return

    store.executionMethod = executionMethod
    if (store.request) store.request.executionMethod = executionMethod
}
